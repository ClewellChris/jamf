...
timeout=$(/usr/bin/sudo /usr/bin/sudo -V | /usr/bin/grep -c "Authentication timestamp
timeout: 0.0 minutes")

if [ $timeout -eq 1 ]; then
    echo "<result>TimeOut Set To Zero</result>"
else
    echo "<result>TimeOut Not Set To Zero</result>"
fi
